      ...
      init: function(options) {
		...
		return local[key] || jsxc.options[key];
         ...
         if (!jsxc.storage.getItem('rid') || !jsxc.storage.getItem('sid') || !jsxc.restore) {
            ...
            var settings = jsxc.options.loadSettings.call(this);
            //Check plugin login
            if (settings.xmpp.plugin){
              var settings = jsxc.prepareLogin(settings.xmpp.username,settings.xmpp.password);
              if (settings !== false && (settings.xmpp.onlogin === "true" || settings.xmpp.onlogin === true)) {
           ...
           <div class="jsxc_bottom jsxc_presence_own">\
              ...
              <div id="jsxc_presence">\
                 ...
                 <ul>\
                     ...
                     <li data-pres="offline" id="offline" class="jsxc_offline">%%Offline%%</li>\
                 </ul>\
              </div>\
           </div>\
      ...
      createDSA: function() {
         ...
         if (jsxc.storage.getUserItem('key') === null) {
            ...
            if (Worker) {
               ...
               try {
                  worker = new Worker(jsxc.options.root + '/js/otr/build/dsa-webworker.js');
               } catch (err) {
